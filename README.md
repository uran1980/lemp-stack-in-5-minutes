...
Chef оперирует **рецептами** (**cookbooks**) - наборами абстрактных определений, описывающих то, как должна быть построена необходимая нам инфраструктура. Т.е. мы в рецептах для Chef указываем, что хотим, чтобы на разворачиваемой системе были созданы определенные пользователи, с определенными правами, установлены и настроены определенные приложения и сервисы. На основе этих рецептов Chef сам, в зависимости от опреционной системы, делает все необходимые действия, чтобы на выходе мы имели готовую к работе систему.
